...
class Carousel02 {
	constructor(){
		this.changing = false;
		this.active = 0;
		this.slider = document.querySelector('.carousel02');
		if(!this.slider) return;
		this.container = this.slider.querySelector('.carousel02__container');
		this.items;
		this.length = this.container.querySelectorAll('.carousel02__slide').length;
		this.prev = this.slider.querySelector('.carousel02__prev');
		this.next = this.slider.querySelector('.carousel02__next');
		this.init();

	}
	init(){
		const group = this.container.querySelector('.carousel02__inner');
		for( let i=0;i<2;i++ ) {
			const clone = group.cloneNode(true);
			this.container.appendChild(clone);
		}
		this.items = this.container.querySelectorAll('.carousel02__slide');
		// 初期位置: アクティブスライドを中央に
	...
	change(number, direction) {
		...
		nextIndex = (nextIndex + this.length) % this.length;
		...
		animation.onfinish = ()=>{
			...
			if( isWrapNext || isWrapPrev ) {
				...
				[
					...
					{
						translate: `${toCenter(nextIndex)}% 0`
					},
				], 
				{
					duration: 0,
